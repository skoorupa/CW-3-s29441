...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeesWithManagers()
    {
        ...
        var withMgr = emps.FindAll(e => e.Mgr != null); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .SelectMany(ed => grades, (ed, g) => new { ed, g })
            .Where(r => r.ed.Sal >= r.g.Losal && r.ed.Sal <= r.g.Hisal)
            .Select(r => new {r.ed.EName, r.ed.DName, r.g.Grade});
